<script lang="ts">
	...
	import { endOfAvailabilitySchema, type EndOfAvailabilitySchema } from './schemas';
	...
	export let data: SuperValidated<Infer<EndOfAvailabilitySchema>>;
  ...
  $: $formData.endOfAvailability = date
	...
	$: value = $formData.endOfAvailability ? parseDate($formData.endOfAvailability) : undefined;
...
<div class="rounded-md border bg-background p-4 text-foreground shadow-sm">
	<div class="flex items-center justify-between">
		<h2 class="text-sm font-medium text-muted-foreground">Fin de la disponibilité</h2>
		<div>
			{#if $isDesktop}
