<script lang="ts">
	...
	import { roomSchema, type RoomSchema } from './schemas';
	...
	export let data: SuperValidated<Infer<RoomSchema>>;
	export let content: string;

	$: number = content;
	$: $formData.numOfRoom = number;
	...
	$: $formData.numOfRoom = content;
</script>

<div class="rounded-md border bg-background p-4 shadow-sm">
	<div class="flex items-center justify-between">
		<h2 class="text-sm font-medium text-muted-foreground max-md:text-xs">Nombre de chambre</h2>
		...
		<div>
			{#if $isDesktop}
				<Dialog.Root bind:open>
					...
					<Dialog.Content>
						<Dialog.Header>
							<Dialog.Title>Nombre de chambres</Dialog.Title>
							<Dialog.Description>Modifier le nombre de chambres</Dialog.Description>
						</Dialog.Header>
						<form action="?/editRoom" method="POST" class="mt-3" use:enhance>
			...
			{:else}
				<Drawer.Root bind:open>
					...
					<Drawer.Content>
						<Drawer.Header>
							<Drawer.Title>Nombre de chambres</Drawer.Title>
							<Drawer.Description>Modifier le nombre de chambres</Drawer.Description>
						</Drawer.Header>
						<form
							action="?/editRoom"
							method="POST"
